...
def parse_risk_classification_response(response_text: str) -> Dict:
    ...
    return {
        'High': ast.literal_eval(response1),
        'Low': ast.literal_eval(response3),
        'counts': {
            'High': len(ast.literal_eval(response1)),
            'Low': len(ast.literal_eval(response3))
        }
    }
...
def create_word_document(pdf_name: str, company_info: str, risk_data: Dict[str, Any],
    ...
    if risk_data['High']:
        ...
        low_risk_heading = doc.add_heading('Low Risk Flags:', level=2)
        low_risk_heading.runs[0].bold = True

        for flag in risk_data['Low']:
            p = doc.add_paragraph()
            p.style = 'List Bullet'
            p.add_run(flag)
...
def main_batch_processing_five_iterations():
    ...
    api_key = "8496bd1da40242a"
    azure_endpoint = "https://crisil-pp-gpt.ope"
    api_version = "2025-01-01-preview"
    deployment_name = "gpt-1-mini"
