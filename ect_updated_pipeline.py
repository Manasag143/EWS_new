...
def deduplicate_high_risk_flags(high_risk_flags, llm):
    """
...
"""
    ...
    try:
        deduplicated_text=llm._call(prompt, temperature=0.1)
        deduplicated_flags = []
...
def process_pdf_enhanced_pipeline_with_per_flag_classification(pdf_path: str, folder_name: str, previous_year_data: str, 
                                llm: AzureOpenAILLM, output_folder: str = "results"):
    """
    ...
    try:        
        ...
        response1 = llm.call_with_system_prompt(system_prompt_step_1,f"%%%%{context}%%%%")
        response2 = llm.call_with_system_prompt(system_prompt_step_2,f"%%%%{context}%%%%")
        response3 = llm.call_with_system_prompt(system_prompt_step_3,f"%%%%{context}%%%%")
        response4 = llm.call_with_system_prompt(system_prompt_step_4,f"%%%%{context}%%%%")

        first_response = response1 + "|" + response2 + "|" + response3 + "|" + response4
        first_part_1= response1 + "|" + response2
        first_part_2= response3 + "|" + response4
...
<review>
        ...
        response1 = llm.call_with_system_prompt(second_system_prompt_A,f"####{second_user_content_A}####")
...
<review>
        ...
        # Use direct client approach
        response2 = llm.call_with_system_prompt(second_system_prompt_B,f"####{second_user_content_B}####")

        second_response = response1 + "|" + response2
...
Provide factual category summaries:"""
        ...
        if high_risk_flags and len(high_risk_flags) > 0:
            ...
            high_risk_flags = deduplicate_high_risk_flags(high_risk_flags,llm)
